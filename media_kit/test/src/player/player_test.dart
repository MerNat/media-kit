...
''';
  ...
  // ─── NetworkConfiguration → mpv property bridge ──────────────────
  //
  // These tests exercise the round-trip from `PlayerConfiguration.network`
  // all the way down into mpv's actual property store. They use
  // `NativePlayer.getProperty` to read back what mpv actually has,
  // which is the only honest way to verify our config landed: a unit
  // test on `buildStreamLavfOptions()` proves the *string* is correct,
  // but says nothing about whether the native init code actually
  // forwards it. These do.
  //
  // Skipped on web — the web backend uses HTML5 video and does not
  // honour mpv-style properties.
  test(
